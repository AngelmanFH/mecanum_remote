...
class DraggableCircleWidget(QWidget):
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if self.dragging:
            ...
            new_center.setX(max(50 + self.circle_radius, min(new_center.x(), 350 - self.circle_radius)))
            new_center.setY(max(50 + self.circle_radius, min(new_center.y(), 350 - self.circle_radius)))
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        self.dragging = False
        self.timer.start(50)  # Start the timer to update every 50 ms (approx. 20 FPS)
...
if __name__ == "__main__":
    app = QApplication([])
    widget = DraggableCircleWidget(position_callback)
    widget.show()
    app.exec()
